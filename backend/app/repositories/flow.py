...
class FlowRepository:
    ...
    async def create(self, flow_name: str, user_id: int) -> Flow:
        flow = Flow(
            flow_name=flow_name,
    ...
    async def get_by_id_for_user(self, flow_id: int, user_id: int) -> Flow | None:
    ...
    async def list_by_user(self, user_id: int) -> list[Flow]:
        result = await self._session.execute(
            select(Flow)
